...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if i == 0 and filters == 3:  # normalize RGB image 对原始RGB图像归一化
            ...
            layers = mdef['layers']
            filters = sum([output_filters[l + 1 if l > 0 else l] for l in layers]) #卷积核数目
            ...
            modules.add_module(
                ...
                kernel_size = int(mdef['kernelsize'])
                ))
        else:
...
class Darknet(nn.Module):
    ...
    def forward(self, x, augment=False, verbose=False):
        ...
        if not augment:
            ...
            # 放大图像 仅推理和测试
            # Augment images (inference and test only) https://github.com/ultralytics/yolov3/issues/931
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        if self.training:  # train
            # 训练状态下，直接输出YOLO要求的Tensor 3*(class+5)
            return yolo_out
            ...
            x = torch.cat(x, 1)  # cat yolo outputs 拼接 第二特征纬度
