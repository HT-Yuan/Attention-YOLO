...
def build_targets(p, targets, model):
    ...
    # targets - [image,class,x,y,w,h] iamge代表当前batch的第几张图片
    # x,y,w,h 已经进行了归一化 
    nt = targets.shape[0] # image
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        # anchor 实际上为除去32之后的结果(以第一层 dowmsample rate=32)为例子
...
# 适应度函数 x 为 Precision recall map f1 w 为权重
def fitness(x):
