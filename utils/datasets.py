...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # 异常 try:可能产生异常的代码块 except:异常处理 后常接raise Exception唤起异常
        try:
            path = str(Path(path))  # os-agnostic
            ...
            # os.path.splitext(x)[-1].lower()为文件后缀小写 语句目的是将图片文件名转为系统合法路径
        ...
        # np.arange(n) 为列表[0 1 n] np.floor 向下取整 bi即可为每一batch的坐标 ex [0 0 1 1 2 2 ...]
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index
        # np 及取bi的最后一个直 加1 表示批次的个数
        ...
        self.rect = False if image_weights else rect # 是否根据权重进行采样 如果是 则无法使用矩形训练
        self.mosaic = self.augment and not self.rect  # load 4 images at a time into a mosaic (only during training)

        # Define labels
        # 标签文件是通过images替换为labels，jpg替换为txt得到的
        ...
        if self.rect:
            ...
            # argsort函数返回的是数组值从小到大的索引值 根据长宽比排序
            irect = ar.argsort()
            ...
            # 设置训练的图片形状 np 为batch批数
            shapes = [[1, 1]] * nb
            for i in range(nb):
                # ari为bactch宽度的数组array 内容为长宽比
                ari = ar[bi == i]
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            x = self.labels[index]
            if x.size > 0:
                # Normalized xywh to pixel xyxy format 将归一华后的xywh转换为左上角 有下角的表达方式
                labels = x.copy()
