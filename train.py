...
def train(hyp):
    ...
    # 线程个数 cpu_count为CPU数量 batch_size 8 三数字中的最小值
...
if __name__ == '__main__':
    ...
    if not opt.evolve:  # Train normally
        ...
        for _ in range(1):  # generations to evolve 迭代数
            if os.path.exists('evolve.txt'):  # if evolve.txt exists: select best hyps and mutate
                ...
                if parent == 'single' or len(x) == 1: # single ：选择上一代最好的一个
                    # x = x[random.randint(0, n - 1)]  # random selection
                    x = x[random.choices(range(n), weights=w)[0]]  # weighted selection
                elif parent == 'weighted': # 选择得分前5加权平均作为下一代
                    x = (x * w.reshape(n, 1)).sum(0) / w.sum()  # weighted combination
 
                # Mutate mathod 选择进化的三种模式 mp突变几率
            ...
            # Clip to limits 为防止突变过程中，出现参数不合理的范围
